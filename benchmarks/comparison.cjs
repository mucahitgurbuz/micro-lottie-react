...
async function compareBundleSizes() {
  ...
  results.forEach(result => {
    ...
    if (baseline && result.name === 'micro-lottie-react') {
      const reduction = ((baseline.gzippedSize - result.gzippedSize) / baseline.gzippedSize * 100).toFixed(1);
  ...
  if (microLottie && baseline) {
    ...
    console.log(`✅ micro-lottie-react is ${factor}x smaller than the average competitor`);
    console.log(`✅ Saves ${((baseline.gzippedSize - microLottie.gzippedSize) / 1024).toFixed(1)}KB of network transfer`);
  }
}
...
async function performanceBenchmarks() {
  ...
  Object.entries(benchmarks).forEach(([metric, data]) => {
    ...
    Object.entries(data).forEach(([lib, value]) => {
      ...
      let improvement = '';
      if (lib === 'micro-lottie-react' && typeof value === 'number' && baseline > 0) {
      ...
      const valueStr = typeof value === 'number' ? `${value}${data.unit}` : value;
...
function featureComparison() {
  ...
  const features = [
    ['Feature', 'micro-lottie-react', 'lottie-react', 'react-lottie', '@lottiefiles/react'],
    ['Bundle Size', '✅ 12KB', '❌ 156KB', '❌ 168KB', '⚠️ 89KB'],
    ['.lottie Support', '✅ Yes', '❌ No', '❌ No', '❌ No'],
    ['Zero Dependencies', '✅ Yes', '❌ No (3 deps)', '❌ No (5 deps)', '❌ No (2 deps)'],
