...
class BundleAnalyzer {
  ...
  getTargetForFile(filename) {
    if (filename.includes("min.js")) return 15; // Production minified bundle
    if (filename.includes("esm") || filename.includes("es")) return 35; // Development ESM
    if (filename.includes("cjs") || filename.includes("common")) return 35; // Development CJS
    if (filename.includes("umd") && !filename.includes("min")) return 40; // Development UMD
    if (filename.includes("react")) return 3;
    if (filename.includes("utils")) return 2;
    return 30; // default for development builds
  }
