...
class PerformanceTester {
  ...
  async testParsingPerformance() {
    ...
    for (const testCase of testCases) {
      ...
      const status =
        actualTime <= testCase.expectedTime ? "✅ PASS" : "❌ FAIL";
  ...
  async testRenderingPerformance() {
    ...
    console.log(
      "Test Case                          | Target FPS | Actual FPS | Frame Time | Status"
    );
    console.log(
      "----------------------------------|------------|------------|------------|--------"
    );
    ...
    for (const test of renderTests) {
      ...
      const status =
        result.fps >= test.expectedFPS * 0.9 ? "✅ PASS" : "❌ FAIL";
      const color = status.includes("PASS") ? "\x1b[32m" : "\x1b[31m";
  ...
  async testMemoryUsage() {
    ...
    console.log(
      "Test Case                          | Expected | Actual | Efficiency | Status"
    );
    console.log(
      "----------------------------------|----------|--------|------------|--------"
    );
    ...
    for (const test of memoryTests) {
      ...
      const efficiency = (
        ((test.expectedMemory - actualMemory) / test.expectedMemory) *
        100
      ).toFixed(1);
      const status =
        actualMemory <= test.expectedMemory ? "✅ PASS" : "❌ FAIL";
      const color = status.includes("PASS") ? "\x1b[32m" : "\x1b[31m";
  ...
  async testLoadingPerformance() {
    ...
    console.log(
      "Test Case                          | Expected | Actual | Improvement | Status"
    );
    console.log(
      "----------------------------------|----------|--------|-------------|--------"
    );
    ...
    for (const test of loadingTests) {
      ...
      const improvement =
        test.name.includes(".lottie") && test.name.includes("JSON")
          ? "50% faster"
          : "-";
  ...
  async testDevicePerformance() {
    ...
    console.log(
      "Device Type                        | Target FPS | Actual FPS | Memory | Status"
    );
    console.log(
      "----------------------------------|------------|------------|--------|--------"
    );
    ...
    for (const test of deviceTests) {
      ...
      const status =
        result.fps >= test.expectedFPS * 0.9 &&
        result.memory <= test.expectedMemory
          ? "✅ PASS"
          : "❌ FAIL";
      const color = status.includes("PASS") ? "\x1b[32m" : "\x1b[31m";
  ...
  generateReport() {
    ...
    Object.entries(testGroups).forEach(([testType, results]) => {
      ...
      const color =
        passRate >= 90 ? "\x1b[32m" : passRate >= 70 ? "\x1b[33m" : "\x1b[31m";
      console.log(
        `${color}${testType
          .toUpperCase()
          .padEnd(
            15
          )}: ${groupPassed}/${groupTotal} passed (${passRate}%)\x1b[0m`
      );
    });
    ...
    console.log("\n" + "=".repeat(60));
    console.log(
      `${overallColor}OVERALL SCORE: ${passedTests}/${totalTests} passed (${overallPassRate}%)\x1b[0m`
    );
  ...
  simulateRenderPerformance(test) {
    ...
    const actualFPS = Math.round(
      targetFPS - variance + Math.random() * variance * 2
    );
  ...
  simulateDevicePerformance(test) {
    const cpuMultiplier =
      test.cpu === "fast" ? 1.2 : test.cpu === "medium" ? 1.0 : 0.7;
    const memoryMultiplier =
      test.memory === "high" ? 0.8 : test.memory === "medium" ? 1.0 : 1.3;

    const fps = Math.round(
      test.expectedFPS * cpuMultiplier * (0.9 + Math.random() * 0.2)
    );
    const memory =
      test.expectedMemory * memoryMultiplier * (0.8 + Math.random() * 0.4);
